...
int LR11();
int LR14(double* array, int size);
int LR16();
int LR19();
...
int LR11() {
    ...
    for (int i = 0; i < 10; i++) {
        float original = A[i];
        // Преобразование: увеличить на 1 значение каждого второго элемента массива
        if (i % 2 == 1) {
            A[i] += 1.0;
        }
...
int analyze_array() {
    ...
    printf("Введите число A  для поиска:\n", A);
...
void tabulateFunction() {
    ...
    printf("Табулирование функции f(x) = x^2 + sin(5x)\n");
    ...
    fprintf(file, "Табулирование функции f(x) = x^2 + sin(5x)\n");
